...
class AircraftCommands(PX4Interface):
    ...
    def hover(self, time: float = 5.0) -> bool:

        if not self._saved_state:
            self.save_state()

        if not self._saved_start_time:
            self.save_start_time()
        
        if self._saved_state and self._saved_start_time:
            elapsed_time = (self.get_clock().now() - self._start_time).nanoseconds / 1e9

            if elapsed_time < time:
                self.goto_position(self._saved_pos, self._saved_yaw)
                return False
    ...
    def save_start_time(self) -> None:
        if self._saved_start_time:
            self.get_logger().warn("aircraft_commands.py: Overwriting start time")
        
        self._saved_start_time = True
